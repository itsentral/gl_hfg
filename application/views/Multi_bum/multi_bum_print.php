...
foreach ($no_bum as $key => $nobum) {
    ...
    $jarh         = $this->db->query("SELECT * FROM jarh WHERE nomor = '" . $nobum . "' ")->row();
    $cbg          = $this->db->query("SELECT cabang FROM pastibisa_tb_cabang WHERE nocab='" . $jarh->kdcab . "'")->row();

    $jur_kasbank  = $this->db->query("SELECT * FROM jurnal WHERE nomor = '" . $nobum . "' and tipe = 'BUM' and (no_perkiraan like '1102%' or no_perkiraan like '1101%') order by nomor");
    ...
    $data_buk_coa1    = $this->db->query("SELECT * FROM coa WHERE no_perkiraan = '$nokir_kasbank1' and bln='$bln' and thn='$thn' and kdcab='$kode_cabang'")->row();
            ...
            ?>
                ...
                ?>
                    <tr>
                        ...
                        <td class="text-left" style="border-left:none;border-right:none;border-bottom:none;border-top:none; font-size:11px;"><?php echo substr($jarh_detail[$i]["keterangan"], 0, 255); ?></td>
